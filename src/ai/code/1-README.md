...
使用 **Codex CLI** 时遇到编辑权限问题，无法正常执行文件修改操作。这通常是由于权限模式配置不当或审批策略设置错误导致的
...
```tex
仔细阅读并理解源码。我需要在本地启动该项目，你需要完成:
1.项目环境的依赖安装和配置文件，需要配置你就配置
2.启动前后端项目
```
...
```tex
我要进一步探索的是这个项目的整体架构。请你结合源码，帮我详细的分析主要的功能组件以及架构设计，
我需要系统的了解这个项目的整体设计思路、架构模式以及对应的功能模块，我需要你
1.结合源码，找到系统模块的划分，明确每个功能模块都是包含哪些代码目录 或核心文件提供服务的；
2.找到前后端以及外部服务依赖接口的入口文件和各功能模块的集成方式；
3.你需要梳理出各个服务之问的依赖关系，包括各个模块之问的调用关系，外部的服务集成；
4.请输出详细的系统架构说明。
注意:我需要的是一个整体的架构设计，你仅需要查看项目代码的顶层目录，不需要深入到函数级的分析，同时，明确的标识出顶层结构的功能模块
主要是由哪些代码模块提供服务的。
新建一个文件夹，以md文档的形式写入，注意:请使用中文
```
