...
间隙锁（Gap Lock）是 InnoDB 在 **REPEATABLE READ（可重复读）** 隔离级别下的核心锁机制，专门用于锁定索引记录之间的**开区间**（如 `(a, b)`），阻止其他事务在该区间内插入新记录，从根本上解决**幻读**问题。

1. 加锁的基本单位是next-key lock。next-key lock是前开后闭区间。
2. 查找过程中访问到的对象才会加锁。
3. 索引上的等值查询，给唯一索引加锁的时候，next-key lock退化为行锁。
4. 索引上的等值查询，向右遍历时且最后一个值不满足等值条件的时候，next-keylock退化为间隙锁。
...
1. 如果是唯一索引
   1.  update xxx where c=4, 那么只会产生行锁、
   2. where id = 6 for update，6 不存在，落在 (4,7) 之间，进行加锁
2. 如果是普通索引
   1. update xxx where c=4,锁定 1-4后， 还会再寻找 4-7，进行加锁
   2. 如果insert c=6, 则 锁定 4-7区间
   3. 查询where id > 5 for update，锁定 (4,7]`、`(7,10]`、`(10,+∞)

# 长事务避免

1. 监控 information_schema.Innodb_trx 表，设置长事务阈值，超过就报警 / 或者 kill；
2. 使用Percona工具的 pt-kill 
3. 在业务层处理
...
1. A线程查询语句，添加读锁, B线程再次查询，添加读锁，正常执行

```sql
select * from t limit 1;
```

2. C线程修改表结构，添加写锁，因为前面有读锁没释放，所以阻塞

```sql
alter table t  xxx;
```

3. D线程读取数据，想获取读锁，阻塞，因为前面的写锁没释放

> 如何解决？

1. 在 MySQL 的information_schema 库的 innodb_trx 表中，你可以查到当前执行中的事务
   1. 查询到这个事务，可以选择kill
2. 可以在alter table里设置等待时间
