...
默认情况下事务每次提交的时候都会刷事务日志到磁盘中，**这是因为变量 innodb_flush_log_at_trx_commit 的值为1**。
...
> 如果节点 A 和 B 之间总是互为主备关系，这种双M的结构
>
> A->B   B产生binlog再到A， A怎么知道这条语句是自己产生的而不产生循环问题呢
...
![image-20260312201323068](3-file/image-20260312201323068.png)

## 写入流程

1. 事务执行过程中，先把日志写到 binlog cache，事务提交的时候，再把 binlog cache 写到 binlog 文件中
2. 每个线程有自己 binlog cache，但是共用同一份 binlog 文件
   1. write，指的就是指把日志写入到文件系统的 page cache
   2. fsync，才是将数据持久化到磁盘的操作
   3. sync_binlog=0 的时候，表示每次提交事务都只 write，不 fsync；
...
3. 执行A事务语句
   1. 因为update是当前读，所以，查到的k=3而不是=2
   2. 如果没有update语句，则是快照读，此时k=1
