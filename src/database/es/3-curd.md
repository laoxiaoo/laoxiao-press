...
如果`operator`的选项是and，则表示XXX的分词必须完全匹配，如果是or，则只需要匹配一个即可
...
定义父子结构，定义`class`为父结构，`student`为子结构

`class_student_field`：自定义的父子关联结构的字段名

`relations`： 关联结构， `class`为父名称，`student`为子名称
...
插入一条班级记录（父数据），并且，自定义了`_id=c_2`， class_student_field的name  = class 表示父结构数据
...
`class_student_field`：name=student表示子结构，并且parent=父文档的_id
...
5.0版本之前，ES是通过`quorum机制`和`timeout机制`来保证一致性的
