...
1. `EventLoop`本质是一个单线程执行器（同时维护了一个Selector)，里面有run方法处理Channel上源源不断的io事件。

2. 其实相当于我们我们[NIO笔记里的worker](/java/java-io/1-nio?id=%e5%a4%9a%e7%ba%bf%e7%a8%8b%e7%89%88%e6%9c%ac)定义

3. 服务器`EventLoop`处理了某个客户端的`channel`连接后，就会绑定，以后处理这个客户端的这个`channel`都是这个线程来处理
...
在构建`group`的时候，我们可以设置两个`group`

1. 第一个表示boss线程池，用于处理`ServerSocketChannel`的`accept事件`通常设置一个
2. 第二个表示worker线程池，用于处理`SocketChannel`的`read和writer事件`
...
`closeFuture()`： 用于处理channel的关闭

`pipeline()`： 用于添加处理器

`write(Object msg, ChannelPromise promise)`： 将数据写入

`writeAndFlush(Object msg)`：将数据写入并刷出

> ChannelFuture的连接问题

**为什么调用connect后，需要调用io.netty.channel.ChannelFuture#sync方法？**

1. 因为`connect`是main线程调用，但真正执行连接的是NioEventLoop的线程去执行的，如果不调用sync就会不等连接完，就结束了当前main线程
...
`等到连接建立完成，调用`operationComplete`方法`
...
2. 通过同步的方式（`channelFuture.syn`）关闭
...
*NioEventLoopGroup#shutdownGracefully*方法能够优雅的结束`NioEventLoopGroup`里的线程
...
由图可见，netty中的future和promise是jdk`future`的升级版本
...
1. `ChannelInboundHandler`入站handler：主要用来读取客户端数据，写回结果

2. `ChannelOutboundHandler`出站handler：主要对写回结果进行加工
...
- `pipeline`的链表是一个双向链表，*Inbound*从*head*往后面执行`channelRead`方法，而 *Outbound*则从*tail*往前面执行`write`方法
...
如：只有上游传输的是Student的对象，才会调用`channelRead0`方法
...
可以运用：`EmbeddedChannel`来进行handle的模拟调用
...
2. 调用`release`方法计数减1，如果计数为0，ByteBuf内存被回收
3. 调用`retain`方法计数加1，表示调用者没用完之前，其它handler即使调用了release 也不会造成回收
...
但是netty提供了一个`slice`方法，直接对ByteBuf中的内容切片，建立虚拟的指针，直接指向ByteBuf的内存，从而达到不需要复制的目的
...
- 这里不会发生数据的复制
- 这里只是逻辑上将其聚合到一起
- `addComponents`将第一个参数设置为true，表示在聚合的过程中自动的增长写指针
- 调用`addComponents`方法也需要考虑调用`retain`方法来进行引用计数的增加，防止被意外的release掉
